...
<template>
  <aside class="border-r  flex flex-col transition-all duration-300">
    ...
    <div @click="controlAside.toggleAside" class="h-12 hover:bg-red-100 flex items-center justify-start cursor-pointer">
      <li class="px-4 list-none flex gap-1 items-center">
        <Icon v-show="controlAside.isAsideOpen" size="24" name="arrow-shape-left-to-line" />
        <Icon v-show="!controlAside.isAsideOpen" size="24" name="arrow-shape-right-to-line" />
      </li>
    </div>

    <div class="flex-1">
      <NavItem :isExpanded="controlAside.isAsideOpen" v-for="item in navList" :key="item.path" :dataItem="item" />
    </div>
    <div class="">
      ...
      <div class="h-12 flex items-center hover:bg-red-100 cursor-pointer">
        <li class="px-4 list-none flex gap-1 items-center">
          ...
          <span v-show="controlAside.isAsideOpen">{{ username }}</span>
        </li>
      </div>
      ...
      <div @click="logout" class="h-12 flex items-center hover:bg-red-100 cursor-pointer">
        <li class="px-4 list-none flex gap-1 items-center">
          ...
          <span v-show="controlAside.isAsideOpen">Cerrar sesión</span>
        </li>
      </div>
    </div>
  </aside>
